-- FUNCTION: public.calculate_cfd
DROP FUNCTION IF EXISTS public.calculate_cfd(
	CHARACTER VARYING,
...
-- DELIMITER //
CREATE
OR REPLACE FUNCTION public.calculate_cfd(
		...
		from
			...
			lateral (
				select
					...
					count("workItemId") as numberofitems --            		, STRING_AGG( "workItemId" , ', ')
				from
					formatted_events
				where
...
ALTER FUNCTION public.calculate_cfd(
	CHARACTER VARYING,
	TIMESTAMP WITH TIME ZONE,
	TIMESTAMP WITH TIME ZONE,
	CHARACTER VARYING,
