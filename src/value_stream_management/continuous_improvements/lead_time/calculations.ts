...
export class Calculations {
    ...
    private async getCompletedItemsSortedByLeadTime() {
    ...
    async getLeadTimes(): Promise<Array<number>> {
    ...
    async getRawCompletedItemsWithWholeDays(): Promise<StateItem[]> {
        const completedItems: StateItem[] = await this.getCompletedItems(StateCategory.COMPLETED);
     ...
     *
     * 2. calculate how many workitems types achieved expectation in SLE
     **/
    getLeadtimeByWorkItemTypeWithSLE(
        defaultCompletedItems: StateItem[],
        defaultSLEConfigItems: WorkItemTypeItem[],
    ): WorkitemTypeWithServiceLevelPercent[] {
        const itemTypeReturned: Set<string> = new Set();

        let allSLEConfigItems = defaultSLEConfigItems;
        if (this.filters && this.filters.workItemTypes) {
            allSLEConfigItems = allSLEConfigItems.filter(
                (item: { id: string; }) =>
                    (this.filters!.workItemTypes as Array<string>).includes(
                        item.id,
                    ),
        ...
        const completedItems: StateItem[] = defaultCompletedItems;
        const workItemsWithServiceLevelPercent: WorkitemTypeWithServiceLevelPercent[] = allSLEConfigItems.map(
            (itemType: WorkItemTypeItem) => {
                const rawLeadTimesPerWeek = completedItems.sort(
                    (
                        a: { [index: string]: any; },
                        b: { [index: string]: any; },
                    ) => {
                ...
                const filterRawLeadtime = rawLeadTimesPerWeek.filter(
                    (item: { flomatikaWorkItemTypeId?: string; }) => item?.flomatikaWorkItemTypeId === itemType.id,
                );

                const formattedRawLeadtime = filterRawLeadtime.map(
                    (item: {
                        [key: string]: any;
                        departureDate?: string;
                        arrivalDate?: string;
                        ...
                        itemTypeReturned.add(itemType.id);
                        return {
                            week: DateTime.fromISO(item.departureDate!)
                                ?.weekNumber,
                            leadtime: item?.leadTimeInWholeDays,
                        };
                    },
                );
                ...
                if (formattedRawLeadtime.length > 0) {
                    let achievedLength: number[] = formattedRawLeadtime.map(
                        (item: { leadtime: number; }) => item.leadtime,
                    );
                    achievedLength = achievedLength.filter(
                        (item) =>
                            item <= itemType.serviceLevelExpectationInDays!,
                    );
                    targetMet = roundToDecimalPlaces(
                        achievedLength.length / formattedRawLeadtime.length,
                        2,
                    );
                }
        ...
        return workItemsWithServiceLevelPercent.filter(
            (response) => itemTypeReturned.has(response.itemTypeId),
        );
    }
}
